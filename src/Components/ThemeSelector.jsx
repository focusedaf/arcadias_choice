...
const ThemeSelector = () => {
  ...
  // Function to create pixelated border CSS
  const createPixelBorder = (color) => {
    return {
      ...
      boxShadow: `
        ...
        /* Additional pixels for more elaborate corners */
        -8px -4px 0 0 ${color},
        -4px -8px 0 0 ${color},
        8px -4px 0 0 ${color},
        4px -8px 0 0 ${color},
  ...
  return (
    <div className="min-h-screen py-12 rounded-xl bg-gradient-to-br from-purple-900 to-pink-900 bg-fixed" style={{
      ...
      <div className="relative mb-30">
        <h2 className="relative z-10 nes-container is-rounded p-4 inline-block mx-auto text-3xl font-['Press_Start_2P'] text-center bg-white/10 backdrop-blur-sm">
          ...
          {themes.map((theme) => (
            <div key={theme.id} className="relative transform hover:scale-105 transition-all duration-300">
              <button
                ...
                className={`group relative p-6 text-white bg-gradient-to-br ${theme.bgClass} overflow-hidden w-full h-full`}
                style={{
                  imageRendering: "pixelated",
              ...
              >
                ...
                <div className="relative z-10">
                  <div className={`text-5xl mb-6 ${theme.animation}`}>{theme.icon}</div>
                  <div className="bg-white/20 backdrop-blur-sm rounded-md p-4 border border-white/30 transform transition-all duration-300 group-hover:bg-white/30">
                    <h3 className="text-lg font-['Press_Start_2P'] mb-3 text-white tracking-wide leading-relaxed">
                      {theme.name}
                    </h3>
                    <p className="text-sm opacity-90 font-['Press_Start_2P'] leading-relaxed">{theme.description}</p>
                  </div>
                </div>
        ...
        <div className="mt-12 text-center">
          <button 
            ...
            className="nes-btn relative text-white font-['Press_Start_2P'] text-sm tracking-wide hover:scale-105 transition-all duration-300"
          >
            ⟵ Return Home
          </button>
        </div>
